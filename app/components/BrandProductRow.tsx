...
export default function BrandProductRow({
  ...
  return (
    <div className="w-full max-w-[1280px] mx-auto py-20 px-4 lg:px-8">
      <div className={`flex flex-col ${isReversed ? "lg:flex-row-reverse" : "lg:flex-row"} items-center gap-10 lg:gap-20`}>

        {/* Image Section */}
        <div className="w-full lg:w-1/2 flex justify-center max-w-[500px]">
          <div className="relative w-full aspect-[3/2] bg-white border border-gray-200 flex items-center justify-center p-8">
            <img
        ...
        <div className={`w-full lg:w-1/2 flex flex-col ${isReversed ? "lg:items-end lg:text-right" : "lg:items-start lg:text-left"} text-center`}>
          ...
          <h2 className="text-[28px] lg:text-[35px] text-brand-navy mb-2 uppercase font-bold">
            {name} <span className="text-xl lg:text-[20px] normal-case ml-2 font-normal">{subtitle}</span>
          </h2>
